...
export default function ManageAgentsPage() {
    ...
    const [isApplicationsFlyoutOpen, setIsApplicationsFlyoutOpen] =
        useState(false);
    ...
    useEffect(() => {
        if (authContext.isAuthenticated && !authContext.isLoading) {
            ...
            if (
                !RegisteredApplicationsListStoreData ||
                Object.keys(RegisteredApplicationsListStoreData).length === 0
            )
    ...
    function GetAllRegisteredApplicationsData() {
    ...
    const handleAgentClick = (agent: AgentDataDTO) => {
        ...
        setEditFormData({
            ...
            applicationId: agent.applicationId || 0,
        ...
        setSelectedApplicationId(agent.applicationId || 0);
    ...
    const toggleApplicationsFlyout = (isOpen: boolean) => {
        setIsApplicationsFlyoutOpen(isOpen);
    };
    ...
    const renderManageAgents = () =>
        IsLoadingStoreData ? (
            ...
            <MainLayout contentClassName="p-0" isFullWidth={true}>
                ...
                {isApplicationsFlyoutOpen && (
                    <div className="fixed top-0 right-0 md:right-1/2 md:w-1/2 w-full h-screen z-[60] transition-all duration-500 ease-in-out">
                        <div className="absolute inset-0 bg-gradient-to-r from-blue-600/20 via-indigo-600/20 to-purple-600/20 blur-sm opacity-50 -z-10"></div>
                        <div className="relative h-full bg-gradient-to-br from-gray-900/95 via-slate-900/95 to-black/95 backdrop-blur-xl border-l border-white/10 shadow-2xl">
                            <AssociateApplicationFlyoutComponent
                                isOpen={isApplicationsFlyoutOpen}
                                onClose={() => toggleApplicationsFlyout(false)}
