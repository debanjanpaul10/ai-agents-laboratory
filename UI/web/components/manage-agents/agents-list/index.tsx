...
export default function AgentsListComponent({
    ...
    const getApplicationName = (applicationId: number): string => {
        const app = (registeredApplications as RegisteredApplicationDTO[]).find(
    ...
    const uniqueApplications = useMemo(() => {
        const apps = [
            ...new Set(
                agentsDataList.map((agent) =>
                    getApplicationName(agent.applicationId),
                ),
            ),
        ];
        return apps.sort((a, b) => a.localeCompare(b));
    }, [agentsDataList, registeredApplications]);

    // Filter and sort agents
    const filteredAndSortedAgents = useMemo(() => {
        let filtered = agentsDataList.filter((agent) => {
            const appName = getApplicationName(agent.applicationId);
            // Search filter
        ...
        filtered.sort((a, b) => {
            ...
            switch (sortBy) {
                case "name":
                    ...
                    break;
                case "application":
                    comparison = getApplicationName(
                        a.applicationId,
                    ).localeCompare(getApplicationName(b.applicationId));
                    break;
                case "date":
                default:
                    comparison =
