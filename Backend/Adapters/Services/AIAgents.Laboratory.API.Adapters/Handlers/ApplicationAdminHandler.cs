...
/// The results are then mapped to the appropriate DTOs using <see cref="DomainToResponseMapper"/> before being returned to the caller.</remarks>
/// </summary>
/// <param name="applicationAdminService">The application admin service.</param>
/// <seealso cref="IApplicationAdminHandler"/>
public sealed class ApplicationAdminHandler(
    IApplicationAdminService applicationAdminService) : IApplicationAdminHandler
{
    /// <inheritdoc/>
    public async Task<IEnumerable<BugReportDataDto>> GetAllBugReportsDataAsync(
    ...
    /// <inheritdoc/>
    public async Task<IEnumerable<NewFeatureRequestDataDto>> GetAllSubmittedFeatureRequestsAsync(
        string currentLoggedinUser,
        CancellationToken cancellationToken = default
    )
    {
    ...
    public bool IsAdminAccessEnabledAsync(
        ...
        applicationAdminService.IsAdminAccessEnabledAsync(
            currentLoggedInUser
        );
