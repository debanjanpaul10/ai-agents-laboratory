...
public interface IWorkspacesHandler
{
    ...
    Task<IEnumerable<AgentsWorkspaceDTO>> GetAllWorkspacesAsync(
        string userName,
    ...
    Task<AgentsWorkspaceDTO> GetWorkspaceByWorkspaceIdAsync(
        string workspaceId,
    ...
    /// <summary>
    /// Gets the workspace group chat response asynchronous.
    /// </summary>
    /// <param name="chatRequest">The chat request.</param>
    /// <param name="currentUserEmail">The current logged in user email.</param>
    /// <param name="cancellationToken">The cancellation token.</param>
    /// <returns>The group chat response.</returns>
    Task<GroupChatResponseDto> GetWorkspaceGroupChatResponseAsync(
